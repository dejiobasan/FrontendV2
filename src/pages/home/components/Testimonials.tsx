import { CircleUserRound } from 'lucide-react';
import { motion, type Variants } from 'framer-motion';

const testimonials = [
  {
    name: "Gospel Chinyereugo",
    role: "ALAT, Senior Software Engineer",
    image: <CircleUserRound className="w-8 h-8 text-gray-400 stroke-[1.5]" />,
    testimonial:
      "Quis neque, eu et ipsum amet, vel et. Varius integer enim pellentesque ornare pharetra faucibs arcu. Mauris blandit egestas nibh.",
  },
  {
    name: "Olajide Obasan",
    role: "Lead Software Engineer, VolvLabs",
    image: <CircleUserRound className="w-8 h-8 text-gray-400 stroke-[1.5]" />,
    testimonial:
      "Quis neque, eu et ipsum amet, vel et. Varius integer enim pellentesque ornare pharetra faucibs arcu. Mauris blandit egestas nibh.",
  },
  {
    name: "Oluwatobi Olajide",
    role: "ALAT, Frontend Engineer",
    image: <CircleUserRound className="w-8 h-8 text-gray-400 stroke-[1.5]" />,
    testimonial:
      "Quis neque, eu et ipsum amet, vel et. Varius integer enim pellentesque ornare pharetra faucibs arcu. Mauris blandit egestas nibh.",
  },
  {
    name: "Cosmas Ezejiofor",
    role: "ALAT, Frontend Engineer",
    image: <CircleUserRound className="w-8 h-8 text-gray-400 stroke-[1.5]" />,
    testimonial:
      "Quis neque, eu et ipsum amet, vel et. Varius integer enim pellentesque ornare pharetra faucibs arcu. Mauris blandit egestas nibh.",
  },
  {
    name: "Temitope Ogundipe",
    role: "ALAT, Senior UI/UX Designer",
    image: <CircleUserRound className="w-8 h-8 text-gray-400 stroke-[1.5]" />,
    testimonial:
      "Quis neque, eu et ipsum amet, vel et. Varius integer enim pellentesque ornare pharetra faucibs arcu. Mauris blandit egestas nibh.",
  },
];

const containerVariants: Variants = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.1,
    },
  },
};

const cardVariants: Variants = {
  hidden: { opacity: 0, y: 20 },
  visible: {
    opacity: 1,
    y: 0,
    transition: {
      type: "spring",
      stiffness: 100,
      damping: 15,
    },
  },
};

const Testimonials = () => {
  return (
    <section className="bg-[#F9FAFB] py-24 px-4 md:px-8 lg:px-16 w-full overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-100px" }}
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-3xl md:text-5xl font-bold text-center mb-16 text-[#111827]">
            Testimonials
          </h2>
        </motion.div>
        
        <motion.div 
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 md:gap-8"
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-50px" }}
        >
          {testimonials.map((item, index) => (
            <motion.div 
              key={index} 
              variants={cardVariants}
              whileHover={{ y: -5, transition: { duration: 0.2 } }}
              className={`bg-white rounded-2xl md:w-[420px] p-8 border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] flex flex-col gap-6 cursor-default lg:col-span-2 ${
                index === 3 ? "lg:col-start-2" : ""
              }`}
            >
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center shrink-0">
                  {item.image}
                </div>
                <div>
                  <h3 className="font-semibold text-gray-900 text-lg">{item.name}</h3>
                  <p className="text-sm text-gray-500 font-medium">{item.role}</p>
                </div>
              </div>
              <p className="text-[#111827] leading-relaxed text-base">
                {item.testimonial}
              </p>
            </motion.div>
          ))}
        </motion.div>
      </div>
    </section>
  );
};

export default Testimonials;
